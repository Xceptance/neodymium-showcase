...
/**
 * This showcase explains how to access test data using the features built into the Neodymium framework.<br> By default, Neodymium looks for a data file that
 * has the same name as the test case and is located in the same package in the resources folder.<br> This example shows the reading from a JSON file that does
 * not meet the naming conventions.<br> Therefore it is specified using the @DataFile annotation.<br> The method DataUtils.get() uses the JsonPath here.
 */
...
public class AccessDataViaJsonPathTest extends AbstractTest
{
    ...
    public void test()
    {
        ...
        $("#introduction h1").should(matchText(DataUtils.get("$.teaserMessage", String.class)));
        $("#introduction p").should(matchText(DataUtils.get("$.teaserComment", String.class)));
