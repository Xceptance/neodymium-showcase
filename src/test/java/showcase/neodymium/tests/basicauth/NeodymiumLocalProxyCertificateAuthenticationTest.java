...
public class NeodymiumLocalProxyCertificateAuthenticationTest extends AbstractTest
{
    ...
    /*
     * If the system under test requires HTTPS certificated authentication then using Neodymium Local Proxy as a MITM
     * (Man in the Middle) is one possible approach. All requests are automatically routed through the Local Proxy
     * running in the Neodymium test execution process. The Neodymium Local Proxy uses the provided certificate
     * information to connect with HTTPS when opening the site.
     * 
     ...
     * 
     * CAUTION This test case fails with the shipped default configuration. The configurations neodymium.localproxy and
     * neodymium.localproxy.certificate has to be set from false to true to run it successfully.
     */
    @Before
    public void configurationCheck()
    {
        ...
        Assert.assertEquals("NeodymiumLocalProxyBasicAuthentication: neodymium.localproxy.certificate.password check failed",
